...
<p style="background: white; padding: 10px; width: 200px;">Bienvenido <strong>
  <?php echo $_SESSION["Administrador"];?>
	...
	<div class="vertical-menu" style="float: center;">
			...
			<a href="AgregarExtra.php" style='background-color:#81D5D0; color:#000; text-align:center; letter-spacing:2px;'>Agregar Extras</a>
	...
	<table style="background: #81D5D0; border: 2px solid gray;">
		<tr>
			...
			<td>
				...
				$d = new DAO();
				if (isset($_POST['btn_agregarExt'])) {
					$cod = '';
					$nom = $_POST['txt_nomExt'];
					$pre = $_POST['txt_preExt'];
					$pre1 = $_POST['txt_preExt1'];
					$est = '';
				...
				<?php
				if (isset($_POST['btn_modificarExtra'])) {
					$cod = $_POST['txt_codExt'];
					$nom = $_POST['txt_nomExt'];
					$pre = $_POST['txt_preExt'];
					$pre1 = $_POST['txt_preExt1'];
					$con = mysqli_connect("localhost", "root", "", "rapidsubway");
					$sql = "update extras set NomExt='$nom', PreExt15='$pre', PreExt30='$pre1' where CodExt='$cod'";
					$res = mysqli_query($con, $sql);
					json_encode($res);
					mysqli_close($con);
					echo "<center><table><tr><td><h6 align='center' style='color: #388e3c;font-size:25px;'>Se actualizo correctamente el Extra</h6><td></tr></table></center>";
				}
				?>

				<form action="AgregarExtra.php" method="POST">
					<table align="center" width="100" class="responsive-table">
						<caption>Extras</caption>
						<tr hidden>
							<td>Codigo del Extra</td>
							<td>
								<input type="number" name="txt_codExt" style="background: white;"placeholder="Digite el codigo del Extra nuevo" maxlength="4" min="1">
							</td>
						</tr>
						<tr>
							<td>Nombre del Extra</td>
							<td>
								<input type="text" name="txt_nomExt" style="background: white;"placeholder="Escriba el nombre del Extra nuevo" required>
							</td>
						</tr>
						<tr>
							<td>Precio del Extra 15 CM</td>
							<td>
								<input type="number" name="txt_preExt" style="background: white;"placeholder="Digite el precio de 15 CM del nuevo Extra" required maxlength="4" min="1">
							</td>
						</tr>
						<tr>
							<td>Precio del Extra 30 CM</td>
							<td>
								<input type="number" name="txt_preExt1" style="background: white;"placeholder="Digite el precio de 30 CM del nuevo Extra" required maxlength="4" min="1">
							</td>
						</tr>
						<tr>
							<td>
								<input type="submit" name="btn_agregarExt" onclick="return ConfirmAdd()" value="Agregar Extra" style="color:black; text-transform:uppercase; background: #35DD1F; padding: 10px; border: 1px solid gray; cursor: pointer;">
							</td>
							<td>
								<input type="submit" name="btn_modificarExtra" onclick="return ConfirmMod()" value="Actualizar Datos" style="color:black; text-transform:uppercase; background: #FFF441; padding: 10px; border: 1px solid gray; cursor: pointer;">
								<string>*Para actualizar el Extra se necesita indicar el CODIGO del EXTRA*</string>
							</td>
						</tr>
					</table>
				</form>
				...
				<table align="center" width="100" class="responsive-table">
					...
					for ($i = 0; $i < count($lista); $i++) {
						...
						echo "<td> <a href='AgregarExtra.php?cod_eli=" . $e->getCodigoExt() . "' style='color: #FE3E3E' onclick='return ConfirmDelete()'>Eliminar Extra</a></td>";
						echo "</tr>";
					}
					?>
