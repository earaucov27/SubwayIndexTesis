...
<script type="text/javascript">
	function ConfirmDelete() {
		var respuesta = confirm("¿Estas seguro/a que deseas ELIMINAR este QUESO?");
		if (respuesta == true) {
			return true;
		} else {
			return false;
...
<p style="background: white; padding: 10px; width: 200px;">Bienvenido <strong>
  <?php echo $_SESSION["Administrador"];?>
	...
	<div class="vertical-menu" style="float: center;">
			...
			<a href="AgregarQueso.php" style='background-color:#81D5D0; color:#000; text-align:center; letter-spacing:2px;'>Agregar Queso</a>
	...
	<table style="background: #81D5D0; border: 2px solid gray;">
		<tr>
			...
			<td>
				...
				<form action="AgregarQueso.php" method="POST">
					<table align="center" width="100" class="responsive-table">
						<caption>Queso</caption>
						<tr>
							<td>Codigo del Queso</td>
							<td>
								<input type="number" name="txt_codQue" style="background: white;"placeholder="Digite el codigo del Queso nuevo" required maxlength="4" min="1">
							</td>
						</tr>
						<tr>
							<td>Nombre del Queso</td>
							<td>
								<input type="text" name="txt_nomQue" style="background: white;"placeholder="Escriba el nombre del Queso nuevo" required>
							</td>
						</tr>
						<tr hidden>
							<td>Estado Queso</td>
							<td>
								<input type="text" name="txt_estQue" value='Activo' required>
							</td>
						</tr>
						<tr>
							<td>
								<input type="submit" name="btn_agregarQueso" onclick="return ConfirmAdd()" value="Agregar Queso" style="color:black; text-transform:uppercase; background: #35DD1F; padding: 10px; border: 1px solid gray; cursor: pointer;">
							</td>
							<td>
								<input type="submit" name="btn_modificarQueso" onclick="return ConfirmMod()" value="Actualizar Datos" style="color:black; text-transform:uppercase; background: #FFF441; padding: 10px; border: 1px solid gray; cursor: pointer;">
								<string>*Para actualizar el Queso se necesita indicar el CODIGO del QUESO*</string>
							</td>
						</tr>
					</table>
				</form>
				...
				<table align="center" width="100" class="responsive-table">
					<caption>Quesos Registrados</caption>
					<tr>
						<td>Codigo</td>
						<td>Nombre</td>
						<td>Estado</td>
					...
					for ($i = 0; $i < count($lista); $i++) {
						...
						echo "<td> <a href='AgregarQueso.php?cod_eli=" . $q->getCodigoQue() . "' style='color: #FE3E3E' onclick='return ConfirmDelete()'>Eliminar Queso</a></td>";
						echo "</tr>";
					}
					?>
				</table>
